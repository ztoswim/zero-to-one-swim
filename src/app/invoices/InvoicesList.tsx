...
import { Search, FileText, Zap, CheckCheck, Trash2, ChevronRight, X, Phone, Mail } from 'lucide-react';
    ...
    <div className="space-y-10 animate-in">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-primary-500 transition-colors z-10" />
          <input 
            type="text" 
            value={search}
      ...
      <div className="space-y-4">
        {processedInvoices.map((inv) => (
          <div 
            key={inv.id} 
            className="bg-white rounded-[2rem] p-6 hover:shadow-2xl hover:shadow-slate-200/50 transition-all cursor-pointer group border-2 border-slate-100 shadow-xl shadow-slate-200/30 flex flex-col md:flex-row md:items-center gap-6"
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">
                  {inv.usedCount} / {inv.totalSessions} {t('common.sessions')}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="text-right min-w-[80px]">
                <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-0.5">{t('common.amount')}</p>
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                ...
                <div className="w-9 h-9 rounded-xl bg-white text-gray-400 flex items-center justify-center border border-gray-100 shadow-sm group-hover/pill:bg-primary-50 group-hover/pill:text-primary-600 transition-all">
        ...
        {processedInvoices.length === 0 && (
          <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
            ...
            <h3 className="text-2xl font-black text-gray-900 mb-2">{t('common.noInvoices')}</h3>
          </div>
        )}
      </div>
      ...
      {showDetail && selectedInvoice && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4" onClick={() => setShowDetail(false)}>
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl max-h-[90vh] overflow-hidden shadow-2xl border-2 border-slate-200 animate-in" onClick={(e) => e.stopPropagation()}>
            <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-xl font-black text-gray-900 tracking-tighter flex items-center gap-2">
                <FileText className="w-5 h-5 text-primary-500" /> {t('common.billingDetail')}
              </h3>
            ...
            <div className="p-8 space-y-6 bg-gray-50/50">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{t('common.date')}</p>
                  <p className="font-bold text-gray-900 text-sm">{selectedInvoice.paymentDate}</p>
                </div>
                <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{t('common.status')}</p>
                  <p className="font-black text-green-500 text-sm uppercase">
                    {selectedInvoice.status === 'paid' ? t('common.paid') : t('common.unpaid')}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                <div>
                  <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-2">{t('common.studentInfo')}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{t('common.coachInfo')}</p>
                    <p className="font-black text-gray-900 text-xs">
                      {coaches.find(c => c.id === selectedInvoice.coachId)?.name}
                    </p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{t('common.lessonDetails')}</p>
                    <p className="font-black text-gray-900 text-xs">
              ...
              <div className="p-6 bg-primary-500 rounded-3xl text-white shadow-xl shadow-primary-100 relative overflow-hidden">
                <p className="text-[10px] font-black opacity-60 uppercase tracking-widest mb-2">{t('common.amount')}</p>
                <p className="text-4xl font-black tracking-tighter">RM{selectedInvoice.totalAmount}</p>
              </div>

              <div className="space-y-4">
                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-2">{t('common.lessonsHistory')}</h4>
                  ...
                  {selectedInvoice.relatedLessons?.map((l: any) => (
                    <div key={l.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
                      ...
                      <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${l.status === 'completed' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                        {l.status === 'completed' ? t('common.completed') : l.status}
                      </span>
                    </div>
                  ))}
                </div>
