...
import { Plus, User, Phone, Mail, CreditCard, ShieldAlert, Briefcase, MapPin, Smartphone, GraduationCap } from 'lucide-react';
import { WheelDateInput } from '@/components/WheelDateInput';
import { addCourseAdvisorAction } from './actions';
import { useTranslation } from '@/lib/i18n/useTranslation';

export function AddCourseAdvisorDialog() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await addCourseAdvisorAction(finalData);
    if (result.error) {
  ...
  return (
    <>
      <button 
        ...
        className="h-12 px-8 bg-primary-500 text-white rounded-xl shadow-xl shadow-primary-200 flex items-center justify-center gap-3 font-black transition-all hover:bg-primary-600 hover:-translate-y-0.5 active:scale-95 group"
      >
        ...
        <span className="hidden sm:inline tracking-tighter uppercase text-white">{t('common.newAdmin')}</span>
      </button>

      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={t('common.newAdmin')} size="wide">
        <form onSubmit={handleSubmit} className="bg-gray-100/30 -m-8 p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="name" required className="w-full px-5 py-3.5 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:border-primary-500 text-sm transition-colors" placeholder={t('common.fullName')} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-slate-700 uppercase tracking-wider">{t('common.displayName')} <Req /></label>
                    <input name="nickname" required className="w-full px-5 py-3.5 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:border-primary-500 text-sm transition-colors" placeholder={t('common.alias')} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input name="phone" required className="w-full px-5 py-3.5 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:border-primary-500 pl-5 text-sm transition-colors" placeholder="+60..." />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-slate-700 uppercase tracking-wider">{t('common.email')}</label>
                    <input name="email" type="email" className="w-full px-5 py-3.5 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:border-primary-500 pl-5 text-sm transition-colors" placeholder="advisor@swim.com" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-slate-700 uppercase tracking-wider">{t('common.fullAddress')}</label>
                  <textarea name="address" className="w-full px-5 py-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:border-primary-500 h-[108px] resize-none text-sm transition-colors" placeholder={t('common.fullAddress')}></textarea>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-2">
            <button type="submit" disabled={loading} className="btn btn-primary bg-primary-500 hover:bg-primary-600 border-none px-24 h-16 text-lg font-black tracking-tighter shadow-xl shadow-primary-200 rounded-2xl w-full lg:w-auto uppercase text-white">{loading ? t('common.processing') : t('common.saveAdmin')}</button>
          </div>
        </form>
      </Modal>
    </>
