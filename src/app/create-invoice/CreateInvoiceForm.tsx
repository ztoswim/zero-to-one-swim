        ...
        <div className="space-y-4">
          <div className="card p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div><h2 className="text-xl font-bold text-gray-900">Basic Information</h2><p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Select Student and Staff</p></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-8 pb-8 border-b border-gray-50">
              <div className="form-group"><label className="label">Invoice Number <span className="req">*</span></label><input type="text" value={form.invNumber} onChange={(e) => setForm({...form, invNumber: e.target.value})} className="input-field h-12 text-sm font-black text-primary-600 bg-primary-50/30 border-primary-100" placeholder="INV-XXXXXX" /></div>
              <div className="form-group"><label className="label">Payment Date <span className="req">*</span></label><WheelDateInput value={form.paymentDate} onChange={(val) => setForm({...form, paymentDate: val})} className="h-12 text-sm" /></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <div className="form-group relative">
                <label className="label">Search Student <span className="req">*</span></label>
                <input type="text" value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} className="input-field pl-4 h-12 text-sm" placeholder="Search student..." />
                {studentSearch && (
                ...
                <div className="md:col-span-2 pt-2 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="px-5 py-3 bg-white rounded-2xl border-2 border-primary-500 flex items-center justify-between shadow-sm">
                      ...
                      <button onClick={() => setForm({...form, studentId: ''})} className="text-primary-500 font-black hover:scale-125 transition-transform">REMOVE</button>
                    </div>
                    {(() => {
                      const student = students.find(s => s.id === form.studentId);
          ...
          <div className="card p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div><h2 className="text-xl font-bold text-gray-900">Course & Billing</h2><p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Select Plan</p></div>
            </div>
        ...
        <div className="space-y-4">
          <div className="card p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div><h2 className="text-xl font-bold text-gray-900">Schedule Plan</h2><p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Timing and Recurrence</p></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="form-group"><label className="label">First Session Date <span className="req">*</span></label><WheelDateInput value={form.date} onChange={(val) => setForm({...form, date: val})} className="h-12 text-sm" /></div>
              <div className="form-group">
                <div className="flex items-center justify-between mb-2">
                  <label className="label mb-0">First Session Time <span className="req">*</span></label>
                  {(() => {
                    const student = students.find(s => s.id === form.studentId);
                    const dateObj = new Date(form.date);
                    const dayName = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][dateObj.getDay()];
                    const hasSlot = student?.fixedSlots?.some((slot: any) => slot.day === dayName);
                    return hasSlot ? (<span className="text-[8px] font-black bg-primary-500 text-white px-2 py-0.5 rounded-full uppercase tracking-widest animate-pulse">Auto-filled</span>) : null;
                  })()}
                </div>
            ...
            <div className="form-group mb-6">
              <label className="label mb-1">Lesson Duration <span className="req">*</span></label>
        ...
        <div className="card shadow-2xl border-0 overflow-hidden" style={{boxShadow: '0 30px 60px -15px rgba(249, 115, 22, 0.2)'}}>
          ...
          <div className="p-10 space-y-10 bg-white">
            <div className="space-y-8">
              <div className="group"><p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Selected Course</p><p className="text-xl font-black text-gray-900 leading-tight">{currentPkg ? currentPkg.name : 'Select a package...'}</p></div>
              <div className="grid grid-cols-2 gap-8 py-8 border-y border-gray-50">
                <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Coach</p><p className="font-black text-gray-800 text-lg">{coaches.find(c => c.id === form.coachId)?.name || 'Unknown'}</p></div>
                <div className="text-right"><p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Quantity</p><p className="font-black text-gray-800 text-lg">{currentPkg ? currentPkg.lessonCount : 0} Sessions</p></div>
              </div>
            ...
            <button onClick={handleSubmit} className="btn btn-primary w-full py-8 text-2xl font-black shadow-2xl shadow-primary-200 transition-all disabled:opacity-20 group" disabled={formErrors.length > 0}>CONFIRM & SUBMIT <Send className="w-6 h-6 group-hover:translate-x-2 transition-transform" /></button>
          </div>
        </div>
      </div>
